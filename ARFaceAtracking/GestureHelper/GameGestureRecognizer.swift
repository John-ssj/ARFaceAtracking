...
class GameGestureRecognizer: UIGestureRecognizer {
...
//        case notStarted
    ...
    private func isUp() -> Bool {
        ...
        for point in touchedPoints {
            ...
            if (abs(point.x - prePoint!.x) > abs(point.y - prePoint!.y)/3) { errorNum += 1 }
        }
    ...
    private func isLeft() -> Bool {
        ...
        for point in touchedPoints {
            ...
            if (abs(point.x - prePoint!.x)/3 < abs(point.y - prePoint!.y)) { errorNum += 1 }
        }
        if errorNum <= touchedPoints.count / 5 { return true }
        return false
    }
