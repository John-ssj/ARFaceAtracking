...
class GameScene: SKScene {
    ...
    private func setUpScene() {
        ...
        TimeLabel.position = CGPoint(x: self.size.width/2, y: self.size.height-TimeLabelSize.height/2)
        ...
        starLabel.position = CGPoint(x: self.size.width-ScoreLabelSize.width/2-ScoreLabelSize.height-130, y: self.size.height-TimeLabelSize.height/2-40)
        self.addChild(ScoreLable)
        ScoreLable.zPosition = 2
        ScoreLable.position = CGPoint(x: self.size.width-ScoreLabelSize.width/2-100, y: self.size.height-TimeLabelSize.height/2-40)
        ...
        for i in 0..<10 {
            ...
            bloodNode.position = CGPoint(x: (i%5)*70+50, y: Int(UIScreen.main.bounds.height) - (i<5 ? 50:120))
    ...
    func continueGame() {
        ...
        // 3,2,1,0倒数计时后继续游戏
        var t = 3
        let continueNode = SKSpriteNode(imageNamed: "number_1/\(t)")
        ...
        let continueTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { _ in
            t -= 1
            if t>=0 {
                continueNode.texture = SKTexture(imageNamed: "number_1/\(t)")
                continueNode.scale(to: continueNodeSize)
                continueNode.run(SKAction.scale(to: 0.7, duration: 0.8))
            }
        }
    ...
    private func gameLost() {
        ...
        for node in self.children {
            ...
            if let Vnode = node as? SKVirusNode {
                //先全部停止运动，再按照距离远近依次爆炸
                let distance = hypot(GameCenter.shared.magicianPoint.x-Vnode.position.x,
                                     GameCenter.shared.magicianPoint.y-Vnode.position.y)
                let t = distance/200
                Vnode.removeAction(forKey: "autoMove")
                Timer.scheduledTimer(withTimeInterval: TimeInterval(t), repeats: false) { _ in
                    Vnode.dieLost()
                }
            }
    ...
    @objc private func gameWin() {
        ...
        LeftStarsNode.particleSpeed = 600
        ...
        RightStarsNode.particleSpeed = 600
        ...
        Timer.scheduledTimer(withTimeInterval: 8, repeats: false) { _ in
            LeftStarsNode.run(SKAction.fadeAlpha(to: 0, duration: 2)) {
                LeftStarsNode.removeFromParent()
            }
            RightStarsNode.run(SKAction.fadeAlpha(to: 0, duration: 2)) {
                RightStarsNode.removeFromParent()
...
extension GameScene: GameManagerDelegate {
    ...
    func allVirusDied() -> Bool {
        let virusNum = self.children.filter { node -> Bool in
            if node is SKVirusNode { return true }
